...
def create_app():
    ...
    mongo = MongoWrapper(os.environ['DEFAULT_DATABASE'])
    ...
    def add_item_save():
        ...
        mongo.create_new_item(title, description, due_date)
        return redirect('/')
    ...
    def start_item():
        ...
        if not item_id:
            return redirect('/')
        mongo.move_item_to_doing(item_id)
        return redirect('/')
    ...
    def complete_item():
        ...
        if not item_id:
            return redirect('/')
        mongo.move_item_to_done(item_id)
        return redirect('/')
